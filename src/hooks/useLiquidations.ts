 ...
 * Real-time Binance Futures liquidation stream via CF Worker proxy.
 * Ring buffer (max 200) · RAF-gated · mountedRef ✓ · Infinite reconnect ✓
 */
...
function reducer(state: State, action: Action): State {
  if (action.type === 'ADD') {
    return { ...state, events: [...action.events, ...state.events].slice(0, MAX_EVENTS) };
  }
